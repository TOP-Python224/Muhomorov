def parse_sound_file(file_path: str, # Параметр строго позиционный, т.к. путь к файлу выглядит самодостаточно, дополнять его именем параметра избыточно. Также нахождение его на 1-м месте среди параметров функции делает код более читаемым.
                     /,
                     *,
                     format: int, # Остальные параметры строго ключевые, т.к. все они цифровые и похожи между собой, в этом случае использование имени параметра делает код более читаемым.
                     channels: str,
                     sample_freq: int,
                     bit_depth: int) -> str:
